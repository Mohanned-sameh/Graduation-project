...
function Homepage() {
  return (
    <>
      <section
        className="w-full mb-36 flex align-middle justify-center bg-cover bg-no-repeat border-b-4 border-orange-500 shadow-2xl shadow-orange-500"
      ...
      <section className="flex justify-center mx-4 mb-40 py-10 shadow-slate-400 max-sm:shadow-3xl shadow-2xl hover:py-12 max-sm:flex max-sm:flex-wrap max-md:justify-center max-md:gap-10 max-md:flex-wrap max-sm:justify-center max-sm:gap-5 transition-all duration-500  border-y-zinc-600 border-4 border-opacity-50 rounded-full p-4">
        {IMAGES &&
          IMAGES.map((item) => (
            <div key={item.id} className="lg:m-4">
              <img
                src={item.image}
                alt="logo"
                className="aspect-square h-28 max-sm:h-10 max-sm:mr-5 rounded-full hover:h-32 max-md:h-16 mr-4 max-md:mr-4 max-md:hover:h-20 max-sm:hover:h-12 ease-in-out transition-all duration-500"
              />
            </div>
          ))}
      </section>
